...
```bash
pnpm test          # 單元 38 ＋ DB 整合 116 ＋ 端到端驗收 134（里程碑 1 之 20 ＋ 映射 25 ＋ 調整 62 ＋ 工作可靠性 27），共 288 條
```

**跑測試前先停掉 `pnpm dev`**：端到端測試會自己 spawn API（8091／8092／8093／8094）與 worker，
...
- [x] monorepo 骨架（§27 結構）＋ PostgreSQL 16＋10 份 migration（26 張表；可從零重建）
...
- [x] **SLICE-M2-03 背景工作可靠性**：`BackgroundJob` 租約＋`claim_token` fencing＋心跳＋安全重領＋冪等鍵＋錯誤四分類；結果寫入單一交易，`VALIDATING` 成為交易內狀態（`docs/adr/ADR-M2-002.md`）
- [ ] 里程碑 2 後續：PREVIEW CalculationRun ＋ CalculationInputManifest → 證據包
...
tests/       unit｜integration（DB 守衛 116 條）｜acceptance（端到端 134 條）｜fixtures/case-001
