...
```bash
pnpm test          # 單元 50 ＋ DB 整合 191 ＋ 端到端驗收 182（里程碑 1 之 20 ＋ 映射 27 ＋ 調整 62 ＋ 工作可靠性 29 ＋ 計算執行 23 ＋ 證據包 21），共 423 條
```
...
- [x] monorepo 骨架（§27 結構）＋ PostgreSQL 16＋17 份 migration（32 張表；可從零重建）
...
tests/       unit｜integration（DB 守衛 191 條）｜acceptance（端到端 182 條）｜fixtures/case-001
