...
```bash
pnpm test          # 單元 43 ＋ DB 整合 147 ＋ 端到端驗收 157（里程碑 1 之 20 ＋ 映射 25 ＋ 調整 62 ＋ 工作可靠性 29 ＋ 計算執行 21），共 347 條
```
...
- [x] monorepo 骨架（§27 結構）＋ PostgreSQL 16＋12 份 migration（30 張表；可從零重建）
...
tests/       unit｜integration（DB 守衛 147 條）｜acceptance（端到端 157 條）｜fixtures/case-001
