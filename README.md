...
```bash
pnpm test          # 單元 66 ＋ DB 整合 615 ＋ 端到端驗收 427（里程碑 1 之 27 ＋ 映射 40 ＋ 調整 89 ＋ 工作可靠性 29 ＋ 計算執行 33 ＋ 證據包 31 ＋ 工作台身分確認 64 ＋ 期間生命週期 37 ＋ 多基礎 16 ＋ 期間工作台 61），共 1,108 條
...
- [x] monorepo 骨架（§27 結構）＋ PostgreSQL 16＋38 份 migration（可從零重建）
...
- [ ] **MVP 3 剩餘**：現金流（REQ-CFS-001，未完成前 MVP 3 不得關閉）／B-06 折算畫面與 replay 入口／G-03 與 `AMENDED` 取代鏈／對外輸出核對（`OutputProfile`）／結果就緒的正式 Guard ID（CR）
...
tests/       unit（66）｜integration（DB 守衛 615 條）｜acceptance（端到端 427 條）｜fixtures/case-001
