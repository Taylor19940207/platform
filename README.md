...
```bash
pnpm test          # 單元（node:test）＋ DB 整合＋端到端驗收，共 48 條
```
...
- [x] monorepo 骨架（§27 結構）＋ PostgreSQL 16＋4 份 migration（19 張表；可從零重建）
...
- [x] 開發環境：macOS＋Docker Compose 為權威；設定全數走 `.env.local`
- [ ] 里程碑 2：科目映射 → 集團科目 TB → Adjustment 草稿 → PREVIEW CalculationRun → 證據包
...
tests/       unit｜integration（DB 守衛 23 條）｜acceptance（端到端 20 條）
docs/        GOVERNANCE｜BACKLOG｜adr/
```
