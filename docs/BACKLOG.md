...
| 2026-08-12 | **折算結果就緒的守衛編號待回寫基線**。SLICE-M3-03 把期間級判定拆成 `fn_period_fx_input_readiness`（＝基線的 G-07，掛 `ADJ_APPROVED → CALCULATING`）與 `fn_period_fx_result_readiness`（折算結果與調節是否完成，日後掛 `CALCULATING → RECONCILING`）。後者在基線 §25.13 的守衛表中**沒有對應編號**；新增正式 Guard ID 須走 CR，因此本刀先用穩定代碼 `POST_FX_RECONCILIATION_READY`（個別失敗代碼以 `POSTFX_` 為前綴）。基線改版時應正式編號並回寫守衛表與 `output_capability` 欄 | SLICE-M3-03 契約走查 | 需改基線（新增守衛編號）——累積實作證據後開 CR |
