...
| 2026-08-10 | **B-03 確認的 TOCTOU 回應語意**（P2，與 2026-08-06 條目同類，合併處理）。TypeScript 預檢通過後，若另一請求先改變批次狀態或撤銷角色，DB 守衛（`fn_sod07_guard`、identity 遷移白名單、角色有效性列鎖）會正確拒絕且**資料不受污染**，但目前可能向使用者回 500 而非 409。**不得**在搬檔時用中文錯誤文案猜狀態碼——文案會改，代碼不會。應等統一的「DB 穩定機器代碼 → HTTP 狀態」映射落地後一併處理（0022 已建立該模式：`Gxx_NOT_IMPLEMENTED:` 前綴 → 409） | SLICE-M2-04 拆層（B-03） | 不需改基線 |
...
| 2026-08-10 | **provided_by 的真實提供者選單**。`provided_by` 目前等於 `uploaded_by`——**這是現況不是規則**。R1 自己上傳時兩者本就相同；但 R2 代傳時必須選擇真正的資料提供者，否則補件與逾期 KPI 會把 R2 算成提供者。service 已接受呼叫端傳入經驗證的 `providedBy`，缺的是 B-00 的選單與「誰可以被代傳」的驗證 | SLICE-M2-04 拆層（上傳） | 不需改基線 |
| 2026-08-10 | **`/admin/jobs` 尚未模組化**。目前仍在 server.ts；待 B-00 搬遷後移入 admin 模組。`/health` 與 `/login` 屬入口層，保留在 server.ts | SLICE-M2-04 拆層 | 不需改基線 |
